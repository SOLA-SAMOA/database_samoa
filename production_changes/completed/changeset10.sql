...
1) Check the time and timezone setting by executing the following selects from pgAdmin
   a) SELECT now()
   b) SHOW TIMEZONE
   If the date/time displayed in a is wrong, then...
2) Run the following commands to reset the TIMEZONE for the sola and document databases
   a) ALTER DATABASE sola_prod SET TIMEZONE TO 'Pacific/Apia';
   b) ALTER DATABASE sola_prod_documents SET TIMEZONE TO 'Pacific/Apia'; 
3) Close and reopen a new pgAdmin session
4) Check the time and timezone again from pgAdmin. If the date/time is still
   wrong it will be due to outdated timezone files in the postgres installation. 
   Hopefully you won't have a problem with the production db as its Postgres 9.2, 
   but the training version may be out of date. If so, determine the Postgres 
   version (should be 9.1) and download the latest minor release for that version
   (e.g. 9.1.8) from http://www.enterprisedb.com/products-services-training/pgdownload#windows  
   You can run the installer to upgrade the installation to the latest minor release.
   Once thats done, check the timezone again.    
4) Restart the postgresql service 
5) Repeat on the Training database  */
...
BEGIN
  ...
  (SELECT COUNT(s.id) FROM service_in_progress_from s, app_in_progress_from a
   ...
   AND   a.status_code = 'requisitioned')::INT AS on_requisition_from, 
	  ...
	  (SELECT COUNT(s.id) FROM service_lodged s
	   WHERE s.application_id IS NOT NULL)::INT AS lodged,
	 (SELECT COUNT(s.id) FROM app_changed a, application.service s
          WHERE s.application_id = a.id
	  AND   a.status_code = 'requisitioned'
	  AND   s.lodging_datetime < to_date
